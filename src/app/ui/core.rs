...
impl Core {
    ...
    pub fn spawn(self) -> (super::Ui, JoinHandle<()>) {
        ...
        let handle = tokio::spawn(async move {
            ...
            let _ = core
                ...
                .await;
            core.log.info("UI: Initial render of Lists screen");
    ...
    async fn handle_navigate_back(&mut self) -> Result<()> {
        match self.state.view {
            ...
            ViewKind::Feed => {
                ...
                self.log.info("UI: Feed -> Lists");
                self.state.view = ViewKind::Lists;
                self.render_lists().await
            }
            ViewKind::Patch => {
                // From patch back to feed
                self.log.info("UI: Patch -> Feed");
                self.state.view = ViewKind::Feed;
                if let Some(list) = self.state.feed_list.clone() {
                    self.render_feed(list).await
                } else {
    ...
    async fn render_lists(&self) -> Result<()> {
        ...
        let end = start + 20;
        self.log
            .info(format!("Lists: fetching items range {}..{}", start, end));
        let items = self.mailing_list_cache.get_slice(start..end).await?;
        self.log
            .info(format!("Lists: fetched {} items", items.len()));
        self.terminal
            .show(Screen::Lists {
                items,
    ...
    async fn render_feed(&self, list: ArcStr) -> Result<()> {
        ...
        let end = start + 20;
        self.log
            .info(format!("Feed: list={} range {}..{}", list, start, end));
        let items = self
            .patch_meta_cache
            .get_slice(list.clone(), start..end)
            .await?;
        if items.is_empty() {
            ...
            self.log
                .info(format!("Feed: fetched {} items", items.len()));
        }
        self.terminal
            .show(Screen::Feed {
                list,
    ...
    async fn render_patch(&self, list: ArcStr, message_id: ArcStr, title: ArcStr) -> Result<()> {
        ...
        {
            Ok(raw) => {
                self.log.info(format!("Patch: raw chars={}", raw.len()));
                match self.render.render_patch(raw).await {
                    Ok(rendered) => {
                        if rendered.is_empty() {
                            ...
                            self.log
                                .info(format!("Patch: rendered chars={}", rendered.len()));
                        }
                        self.terminal
                            .show(Screen::Patch {
                                title,
                    ...
                    Err(e) => {
                        self.log.error(format!("Patch: render error: {}", e));
                        self.patch_meta_cache.invalidate_cache(list).await;
                        self.terminal
            ...
            Err(e) => {
                self.log.error(format!("Patch: fetch error: {}", e));
                self.patch_meta_cache.invalidate_cache(list).await;
                self.terminal
