...
async fn test_log_flush_with_messages_above_level() {
    ...
    let log = Log::spawn(mock_fs, mock_config).await.unwrap();
...
async fn test_log_flush_with_no_messages() {
    ...
    mock_config.expect_log_level().returning(|| LogLevel::Error);
    mock_config
        .expect_usize()
        .with(mockall::predicate::eq(crate::app::config::USizeOpt::MaxAge))
        .returning(|_| 0);
...
async fn test_log_collect_garbage_with_max_age_zero() {
    ...
    mock_config.expect_log_level().returning(|| LogLevel::Info);
    mock_config
        .expect_usize()
        .with(mockall::predicate::eq(crate::app::config::USizeOpt::MaxAge))
        .returning(|_| 0); // max_age = 0 means no cleanup
...
async fn test_log_collect_garbage_with_old_files() {
    ...
    mock_config
        .expect_usize()
        .with(mockall::predicate::eq(crate::app::config::USizeOpt::MaxAge))
        .returning(|_| 7); // max_age = 7 days

    let log_dir_clone = log_dir.clone();
    mock_config
        .expect_path()
        .with(mockall::predicate::eq(crate::app::config::PathOpt::LogDir))
        .returning(move |_| log_dir_clone.clone());
