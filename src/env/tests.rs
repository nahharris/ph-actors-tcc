...
async fn test_env_operations() {
    ...
    // Test set and get
    env.set_env(key.clone(), value)
        .await
        .expect("Setting environment variable to succeed");
    ...
    // Test unset
    env.unset_env(key.clone())
        .await
        .expect("Unsetting environment variable to succeed");
...
async fn test_set_env() {
    ...
    let result = env
        .env(key.clone())
        .await
        .expect("Getting environment variable to succeed");
    assert_eq!(result.deref(), value);
