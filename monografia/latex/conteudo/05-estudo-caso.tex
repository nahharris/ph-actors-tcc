...
\begin{itemize}
    ...
    \item \texttt{ShowPatch}: Exibe o conteúdo renderizado de um patch específico
    \item \texttt{UpdateSelection}: Atualiza o item selecionado na visão atual
    \item \texttt{PreviousPage}: Navega para a página anterior
    \item \texttt{NextPage}: Navega para a próxima página
    \item \texttt{NavigateBack}: Navega de volta na hierarquia (patch → feed → listas)
    \item \texttt{SubmitSelection}: Submete a seleção atual (abre feed ou patch)
\end{itemize}
