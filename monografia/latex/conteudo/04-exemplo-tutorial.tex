...
outros usuários, mas em conjunto com a \emph{package} \texttt{LaTeX Workshop}
(do editor, não do \LaTeX), é uma boa opção. O mesmo vale para o editor
emacs (\url{www.gnu.org/software/emacs}) e sua package \texttt{AUC\TeX{}}.
...
são \texttt{book}, \texttt{article} e \texttt{beamer} (para apresentações); você pode
...
disponibiliza as classes \texttt{amsart}, \texttt{amsbook} e \texttt{amsproc}; você
...
\ltxcmd{paragraph} (a classe \texttt{book} inclui também \ltxcmd{part}, um nível
...
vários parágrafos). A package \texttt{csquotes} acrescenta recursos sofisticados
para citações.

Para poesia, use \ltxcmd{begin\{verse\}} (a package \texttt{verse} acrescenta
...
\texttt{biblatex}\index{biblatex} e do programa auxiliar
\cmd{biber}\index{biber}\footnote{Antigamente, usava-se a package
\texttt{natbib}\index{natbib} e o comando auxiliar \cmd{bibtex}\index{bibtex}.
...
na documentação da package \texttt{natbib}\index{natbib} \citep{natbib}.

\item A escolha do estilo bibliográfico (usando as opções da package
\texttt{biblatex}) que formata as citações ao longo do texto e gera a bibliografia
...
da package \texttt{biblatex} \citep[em especial as seções 2.1.1 e 2.2.2]{biblatex},
...
podem exportar para o formato .bib. Observe que \texttt{biblatex}
...
que \texttt{biblatex} oferece uma solução.
...
\ltxcmd{begin\{flushright\}} (alinha parágrafos à direita)\footnote{É
                             ...
                             \texttt{ragged2e} (já incluída neste modelo)
...
\begin{itemize}
    ...
    %configuração da package \texttt{inputenc}). Alguns anos atrás,
    ...
    %de \texttt{inputenc}, seja recodificando o arquivo), mas arquivos ``.bib''
    ...
    \item As classes padrão (\texttt{book}, \texttt{article} etc.) não foram criadas
    ...
    classes (\texttt{scrbook} no lugar de \texttt{book}, \texttt{scrartcl} no lugar de
    \texttt{article} etc.) chamado \texttt{KOMA-Script}, com mais recursos e mais
    possibilidades de customização. A classe \texttt{memoir} tem o mesmo objetivo,
