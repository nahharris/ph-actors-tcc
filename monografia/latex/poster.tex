...
\posterbox[footerbox]{name=footerbox, above=bottom, column=1, span=12}{
    \begin{minipage}[t]{0.5\textwidth}
        ...
        \textcolor{imesoftblue!30!white}
          {Departamento de Ciência da Computação --- Universidade de São Paulo}\relax
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.45\textwidth}
        ...
        \vspace{2pt}
        Paulo Roberto Miranda Meirelles\par
        David Barros Tadokoro\par
    \end{minipage}
}
...
\posterbox[adjusted title = Modelo de Atores, smallmargins, equal height group = toprow, colback=white]
          {name=contexto, below=titlebox, column=1, span=6}{

    Teoria matemática originalmente concebida para sistemas distribuídos. 
    
    Enxerga sistemas computacionais em termos de \textbf{atores} autônomos que se comunicam através de \textbf{mensagens}.
...
\posterbox[adjusted title = Objetivo, smallmargins, equal height group = toprow, colback=white]
          {name=objetivo, below=titlebox, column=7, span=6}{

    Este trabalho realiza uma experimentação aplicando o Modelo de Atores em um contexto distante do seu domínio natural: \textbf{aplicações centralizadas em Rust}.

    O objetivo é entender o esforço necessário para aplicar um padrão arquitetural fora do seu contexto natural e quais consequências isso traz para o projeto.
...
\posterbox[adjusted title = Estudo de Caso: Patch-Hub, smallmargins, equal height group = patchhub, colback=white]
          {name=estudocaso, below=contexto, column=1, span=4}{

    \textbf{Patch-Hub} é uma ferramenta de terminal desenvolvida como parte do ecossistema KWorkflow para interagir com a API do Lore Kernel Archive. Para validação, foi realizada uma reescrita do projeto aplicando o Modelo de Atores com as adaptações propostas.
}


%%%%%%%% Espaço para Imagem %%%%%%%%

\posterbox[adjusted title = Original, smallmargins, equal height group = patchhub, colback=white]
          {name=original, below=objetivo, column=5, span=4}{

    \begin{center}
        \includegraphics[width=0.9\textwidth]{figuras/patch-hub-original.png}
...
\posterbox[adjusted title = Reescrito, smallmargins, equal height group = patchhub, colback=white]
          {name=reescrito, below=objetivo, column=9, span=4}{
    \begin{center}
        \includegraphics[width=0.9\textwidth]{figuras/patch-hub-new.png}
...
\posterbox[adjusted title = Arquitetura, smallmargins, colback=white]
          {name=arquitetura, below=estudocaso, column=1, span=5}{
    \begin{center}
        \includegraphics[width=1\textwidth]{figuras/patch-hub-actors.png}
...
\posterbox[adjusted title = Resultados, smallmargins, colback=white]
          {name=resultados, below=original, column=6, span=7}{
    \begin{itemize}
...
\posterbox[adjusted title = Conclusão, smallmargins, colback=white]
          {name=conclusao, below=resultados, column=6, span=7}{

    O principal resultado demonstra que a adequação de padrões arquiteturais pode ser \textbf{surpreendentemente flexível}. Sendo possível extrair valor ao se reimaginar tanto o problema quanto o padrão.
}
