...
\posterbox[adjusted title = Modelo de Atores, smallmargins, equal height group = toprow]
    ...
    O Modelo de Atores enxerga sistemas computacionais em termos de unidades autônomas chamadas de \textbf{atores} que se comunicam entre si exclusivamente através de \textbf{mensagens} assíncronas.

    A comunicação, feita via passagem de mensagens, elimina a necessidade de compartilhamento de memória e \textit{locks}. Este padrão serviu de base para linguagens de programação como Erlang e Elixir, tornando-se o paradigma central dessas linguagens.
}


%%%%%%%% Objetivo %%%%%%%%

\posterbox[adjusted title = Objetivo, smallmargins, equal height group = toprow]
          {name=objetivo, below=titlebox, column=7, span=6}{

    Este trabalho realiza uma experimentação aplicando o Modelo de Atores em um contexto distante do seu domínio natural: \textbf{aplicações centralizadas em Rust}. Rust não possui o Modelo de Atores como paradigma central, diferentemente de linguagens como Elixir.
...
\posterbox[adjusted title = Estudo de Caso: Patch-Hub, smallmargins]
          {name=estudocaso, below=contexto, column=1, span=8}{

    \textbf{Patch-Hub} é uma ferramenta de terminal desenvolvida como parte do ecossistema KWorkflow para interagir com a API do Lore Kernel Archive. A ferramenta permite aos desenvolvedores navegar por listas de patches do kernel Linux, aplicar filtros de busca, visualizar conteúdo de patches e gerenciar seu fluxo de trabalho de revisão.
...
\posterbox[adjusted title = Arquitetura, smallmargins]
          {name=imagem, below=objetivo, column=9, span=4}{

    % Imagem será inserida aqui posteriormente
}


%%%%%%%% Resultados %%%%%%%%

\posterbox[adjusted title = Testabilidade, smallmargins, equal height group = resultados]
          {name=testabilidade, below=estudocaso, column=1, span=4}{

    A abordagem facilita imensamente a criação de testes unitários. Como cada ator possui responsabilidades bem definidas, é fácil criar mocks para suas dependências. A cobertura de testes aumentou cerca de \(3x\), permitindo testes paralelos sem condições de corrida.
}

\posterbox[adjusted title = Extensibilidade, smallmargins, equal height group = resultados]
          {name=extensibilidade, below=estudocaso, column=5, span=4}{
...
\posterbox[adjusted title = Complexidade, smallmargins, equal height group = resultados]
          {name=complexidade, below=estudocaso, column=9, span=4}{

    A base de código aproximadamente \textbf{dobrou} de tamanho comparada à implementação original. Isso é um indicativo de que a abordagem tende a produzir código mais verboso, exigindo cuidados especiais para manter qualidade e legibilidade a longo prazo.
}


%%%%%%%% Conclusão %%%%%%%%

\posterbox[adjusted title = Conclusão, smallmargins]
          {name=conclusao, below=testabilidade, column=1, span=12}{

    O principal resultado demonstra que a adequação de padrões arquiteturais pode ser \textbf{surpreendentemente flexível}. Mesmo utilizando um padrão aparentemente inapropriado para o contexto, foi possível extrair valor significativo da abordagem ao reimaginar tanto o problema quanto o padrão, permitindo que ambos se encaixassem de forma interessante.

    Conclui-se que, embora os padrões arquiteturais possuam domínios naturais de aplicação, eles \textbf{não estão rigidamente limitados} a esses contextos. A exploração criativa de padrões em domínios distantes de sua origem pode resultar em soluções bem-sucedidas, desde que haja disposição para adaptar e reimaginar as abordagens estabelecidas. Naturalmente, esse processo de adaptação demanda planejamento cuidadoso e múltiplas iterações.
}
